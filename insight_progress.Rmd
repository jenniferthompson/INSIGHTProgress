...
attitude_df <- all_enrolled %>%
  mutate(
    attitude_pt_ever = factor(
      as.numeric(attitude_pt_ever), levels = 0:1, labels = c("No", "Yes")
    )
  ) %>%
  add_count(attitude_pt_ever) %>%
  add_count(attitude_pt_status) %>%
  dplyr::select(attitude_pt_ever, attitude_pt_status, n, nn) %>%
  distinct() %>%
  filter(!is.na(attitude_pt_ever)) %>%
  set_names(c("attitude_pt_ever", "attitude_pt_status", "n_ever", "n_status"))

tm_attitude <- treemap(
  dtf = attitude_df,
  index = c("attitude_pt_ever", "attitude_pt_status"),
  vSize = "n_status",
  type = "index",
  title = "",
  algorithm = "squarified",
  palette = c("#571705", "#455705"),
  draw = FALSE
)
